...
template <typename T> struct Subtractable : crtp<T, Subtractable> {
  T operator-(T const &other) const {
    return T(this->underlying().get() - other.get());
  }
};
...
template <typename T> struct Comparable : crtp<T, Comparable> {
...
template <typename Destination> struct ImplicitlyConvertibleTo {
...
template <typename NamedType_> struct FunctionCallable;
...
template <typename NamedType_> struct MethodCallable;
